...
func TestLowCardinalityBorrowedResendRebuildsDictionary(t *testing.T) {
	first := []byte("same")
	second := []byte("same")
	lc := newLowCardinalityString(t)
	require.NoError(t, lc.AppendRow(BorrowedBytes(first)))
	require.NoError(t, lc.AppendRow(BorrowedBytes(second)))

	require.Equal(t, []string{"same", "same"}, encodeAndDecodeLowCardinalityStrings(t, lc))
	second[0] = 'X'
	require.Equal(t, []string{"same", "Xame"}, encodeAndDecodeLowCardinalityStrings(t, lc))
}

func TestLowCardinalityMixedResendRebuildsDictionary(t *testing.T) {
	borrowed := []byte("same")
	lc := newLowCardinalityString(t)
	require.NoError(t, lc.AppendRow("same"))
	require.NoError(t, lc.AppendRow(BorrowedBytes(borrowed)))
...
func TestLowCardinalityBorrowedRewriteRebuildsDictionary(t *testing.T) {
	first := []byte("same")
	second := []byte("same")
	lc := newLowCardinalityString(t)
	require.NoError(t, lc.AppendRow(BorrowedBytes(first)))
	...
	require.Equal(t, []string{"same", "same"}, writeAndDecodeLowCardinalityStrings(t, lc))
	second[0] = 'X'
	require.Equal(t, []string{"same", "Xame"}, writeAndDecodeLowCardinalityStrings(t, lc))
}

func newLowCardinalityString(t *testing.T) *LowCardinality {
	t.Helper()
	col, err := Type("LowCardinality(String)").Column("test", nil)
	require.NoError(t, err)
	return col.(*LowCardinality)
}

func encodeAndDecodeLowCardinalityStrings(t *testing.T, source *LowCardinality) []string {
	t.Helper()
	var encoded chproto.Buffer
	source.Encode(&encoded)
	return decodeLowCardinalityStrings(t, source.Rows(), encoded.Buf)
}

func writeAndDecodeLowCardinalityStrings(t *testing.T, source *LowCardinality) []string {
	t.Helper()
...
func decodeLowCardinalityStrings(t *testing.T, rows int, encoded []byte) []string {
	t.Helper()
	decoded := newLowCardinalityString(t)
	reader := chproto.NewReader(bytes.NewReader(encoded))
	require.NoError(t, decoded.Decode(reader, rows))
	values := make([]string, rows)
	for i := range values {
		values[i] = decoded.Row(i, false).(string)
	}
	return values
}
